<template>
    <ol v-if="breadcrumbs.length" class="hidden lg:flex items-center space-x-3 text-gray-500 dark:text-gray-400">
        <li v-for="(crumb, index) in breadcrumbs">
            <div class="flex items-center">
                <NuxtLink :to="crumb.link"
                    :class="[{ 'hover:text-gray-600 hover:dark:text-gray-300': crumb.link }, { 'font-medium text-gray-900 dark:text-gray-200': index === breadcrumbs.length - 1 }, { 'mr-3': breadcrumbs.length > 1 }]">
                    {{ crumb.title }}
                </NuxtLink>
                <svg v-if="index !== breadcrumbs.length - 1"
                    class="h-5 w-5 flex-shrink-0 text-gray-300 dark:text-gray-600" fill="currentColor"
